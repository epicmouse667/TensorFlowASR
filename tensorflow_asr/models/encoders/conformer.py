...
class ConformerDecoder(tf.keras.Model):
    def __init__(
        ...
        **kwargs
        ...
        self.do = tf.keras.layers.Dropout(dropout, name=f"{name}_dropout_1")
    def call(
        self,
        inputs,
        training=False,
        **kwargs,
        ...
        outputs = self.conv2d_transpose(inputs, training=training, **kwargs)
        B,T,E,C = shape_util.shape_list(outputs)
        outputs = tf.reshape(outputs,[B,T,E*C])
        outputs = self.ln(outputs, training=training, **kwargs)
        outputs = self.do(outputs, training=training, **kwargs)
        outputs = self.ffn(outputs,training=training, **kwargs)
        return outputs
